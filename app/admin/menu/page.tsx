...
export default function AdminMenu() {
  ...
  const openEditModal = (item: MenuItem) => {
    setForm({ ...item, price: item.price.toString(), name_en: item.name_en || "", name_es: item.name_es || "", description_en: item.description_en || "", description_es: item.description_es || "" });
  ...
  return (
    <div className="p-4 md:p-10 bg-black min-h-screen text-white pt-40 md:pt-48">
      
      <AnimatePresence>
        {toast && (
          <motion.div initial={{ opacity: 0, y: 50, x: 20 }} animate={{ opacity: 1, y: 0, x: 0 }} exit={{ opacity: 0, scale: 0.5 }} className={`fixed bottom-10 right-10 z-[100] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border backdrop-blur-md ${toast.type === 'success' ? 'bg-neutral-900/90 border-green-500/50 text-green-400' : 'bg-neutral-900/90 border-red-500/50 text-red-400'}`}>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-12 mt-4">
          <div>
        ...
        <div className="relative mb-8">
          ...
          <input type="text" placeholder="Rechercher un plat ou une catégorie..." className="w-full bg-neutral-900 border border-neutral-800 rounded-2xl py-4 pl-12 pr-4 text-white focus:border-kabuki-red outline-none shadow-xl transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-3xl overflow-hidden backdrop-blur-sm shadow-2xl">
          {loading ? (
            <div className="p-20 text-center text-gray-500 flex flex-col items-center gap-4">
              ...
              <p className="italic uppercase text-[10px] tracking-widest">Chargement des données...</p>
            </div>
          ) : (
                  ...
                  {filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-5 flex items-center gap-4">
                        <Image src={item.image_url || "/placeholder-sushi.jpg"} alt={item.name_fr} width={48} height={48} className="rounded-xl object-cover bg-neutral-800 border border-neutral-800 shadow-lg" />
                        <div>
                          <div className="font-bold text-sm text-white">{item.name_fr}</div>
                      ...
                      <td className="p-5 text-center font-mono text-sm text-kabuki-red font-bold">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-800 p-6 md:p-8 rounded-3xl max-w-4xl w-full shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8 border-b border-neutral-800 pb-4">
              ...
              <div className="flex items-center gap-3">
                <button type="button" onClick={handleTranslate} disabled={isTranslating} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition disabled:opacity-50">{isTranslating ? <Loader2 className="animate-spin" size={14}/> : <Wand2 size={14}/>} Traduire</button>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div><label className="text-[10px] uppercase text-gray-500 font-bold mb-2 block tracking-widest">Catégorie</label>
                  <select className="w-full bg-black border border-neutral-800 p-3 rounded-xl outline-none focus:border-kabuki-red transition" value={form.category} onChange={e => setForm({...form, category: e.target.value})}>
                    <option>Les Signatures (Créations Kabuki)</option><option>Makis</option><option>Sushis</option><option>Entrées & Accompagnements</option><option>Box à Partager</option><option>Burrito Sushi (Format généreux)</option>
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="border-2 border-dashed border-neutral-800 p-6 rounded-2xl text-center hover:border-kabuki-red transition-colors group relative">
                {form.image_url ? (
                  <div className="relative group"><Image src={form.image_url} alt="Aperçu" width={150} height={128} className="mx-auto rounded-lg object-cover shadow-xl" /><button type="button" onClick={() => setForm({...form, image_url: ""})} className="absolute top-0 right-0 bg-red-600 rounded-full p-1 shadow-lg translate-x-1/2 -translate-y-1/2"><X size={12}/></button></div>
                ) : (
                  <><Upload className="mx-auto mb-2 text-gray-600 group-hover:text-kabuki-red transition-colors" /><label htmlFor="image-upload" className="cursor-pointer text-sm font-bold text-gray-400 group-hover:text-white transition-colors">{uploading ? "Envoi en cours..." : "Cliquez pour uploader une photo"}</label><input id="image-upload" type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={uploading} /></>
                )}
              </div>
              <button type="submit" disabled={actionLoading || uploading || isTranslating} className="w-full bg-kabuki-red text-white py-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-red-700 transition-all flex items-center justify-center gap-3 shadow-xl shadow-red-900/20 disabled:opacity-50">
